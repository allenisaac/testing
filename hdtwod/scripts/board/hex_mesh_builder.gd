    ...
    for i in range(6):
        ...
        match edge_type:
            ...
            BoardUtils.EdgeType.CLIFF:
               # Always extend walls to the absolute world floor regardless of
                # neighbour elevation, so no tile appears to float.
                var wall_bottom_y := HexCoord.elevation_to_height(0) - HexCoord.ELEVATION_STEP
                _add_cliff_edge_sector(					st,
                    st_side,
                    inner[i], inner[next_i],
                    outer[i], outer[next_i],
                    top_y,
